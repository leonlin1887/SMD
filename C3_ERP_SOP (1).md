...
## 附錄：2018 版 vs 2025 版系統差異對照

> **說明：** 本 SOP 以 2025-04-17 操作影片為準。以下對照 2018-09-11 舊版操作手冊（83頁），標註主要差異。

### 模塊名稱變化

| 2018 舊版 | 2025 新版（當前） | 變化說明 |
|:----------|:-----------------|:---------|
| Sales Entry (Invoice) P25-27 | **Delivery Order** + **Commercial Invoice** | 分拆為兩個獨立模塊 |
| 無此模塊 | **PO Inbound** | ✅ 新增 — 採購入庫獨立管理 |
| 無此模塊 | **DO Outbound (Instruction)** | ✅ 新增 — 出貨物流安排 |
| 無此模塊 | **Pack Multiple PO** | ✅ 新增 — 多 PO 打包合併 |
| Order Entry P20-23 | Order Entry（保留） | 命名一致，功能增強 |
| Order P/O Entry P31-32 | Order P/O Entry（保留） | 命名一致 |

### 功能差異

| 功能點 | 2018 舊版 | 2025 新版 |
|:-------|:----------|:----------|
| 銷售出貨 | 叫「Sales Entry (Invoice)」，出貨+發票同一個頁面 | 分為獨立的 Delivery Order 和 Commercial Invoice |
| 入庫管理 | 無獨立模塊 | 新增 PO Inbound，支持 Import POList 導入 |
| 物流安排 | 無此功能 | 新增 Out Process → DO Outbound，含物流公司、ETD/ETA |
| 打包作業 | 無此功能 | 新增 Pack Multiple PO + Merge CtnInfo |
| 報表生成 | 系統內建列印 | 集成 SAP Crystal Reports（「文件處理中，請稍候」）|
| 系統版本 | C3 Enterprise（無年份標示） | C3 - New Ver.2025 |
| 開發公司 | 東莞市艾提思**電腦**有限公司 | 東莞艾提思**軟件科技**有限公司 |

### 操作流程演變

**2018 年流程（簡化）：**
```
Order Entry (SO) → Sales Entry (Invoice) [出貨+發票合一] → PackingList → Order P/O (PO)
```

**2025 年流程（當前，支持影像）：**
```
Order P/O Entry → [PO Inbound →] Delivery Order → Commercial Invoice → Packing List → DO Outbound → Pack Multiple PO
```

### SOP 適用說明

- **章節 1-2（PO Entry / PO Inbound）：** PO Inbound 為 2025 新增，舊手冊無對應內容
- **章節 3（Delivery Order）：** 舊手冊叫「Sales Entry (Invoice)」，舊操作為從 SO 導入數據做「出貨」，發票在同一頁生成
- **章節 4（Commercial Invoice）：** 舊手冊中與 Delivery Order 在同一頁面，新版本獨立
- **章節 5（Packing List）：** 命名一致，但 2025 版集成 SAP Crystal Reports 生成正式報表
- **章節 6（DO Outbound）：** 全新模塊，2018 無此功能
- **章節 7（Pack Multiple PO）：** 全新模塊，2018 無此功能

*參考來源：2018-09-11 島田商事 C3 Enterprise 功能及操作說明（管理番號 tw-1809002）*

---

*本 SOP 基於 C3 Enterprise ERP 操作錄影分析生成（2025-04-17），以新影片為準*
